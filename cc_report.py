...
This is the same branded report format established in the session,
regenerated fresh each time new data arrives.
"""
...
def load_report_data(ledger_path: str) -> tuple[list[dict], dict]:
    """Load ledger and precompute all aggregations for the report."""
    rows = []
    with open(ledger_path, newline="", encoding="utf-8-sig") as f:
        reader = csv.DictReader(f)
        for row in reader:
            amt = float(row.get("Amount (Rs.)", 0) or 0)
            is_credit = row.get("Is Credit") == "TRUE"
    ...
    # Best rates: vendors with points, sorted by rate DESC then amt DESC
    ...
    for desc, v in v_pts.items():
        if v["count"] >= 2 and v["amt"] > 1000:
            ...
            best_rates.append({
                "desc": desc[:45], "rate": round(rate, 2),
                "amt": round(v["amt"], 2), "pts": v["pts"],
                "count": v["count"], "cat": v["cat"]
            })
    best_rates.sort(key=lambda x: (-x["rate"], -x["amt"]))

    # Worst rates: ALL vendors (incl zero), sorted by rate ASC then amt DESC
    ...
    precomputed = {
        ...
        "by_subcat": [
            {"cat": v["cat"], "subcat": v["subcat"], "amt": round(v["amt"], 2),
...
def build_report_html(precomputed: dict, config: dict, today_str: str) -> str:
    """Build the full my-cc-report HTML string."""
...
<title>my-cc-report · {card_name}</title>
...
.rate-card{{background:#fff;border:1.5px solid var(--border);border-radius:10px;padding:0;overflow:hidden;transition:box-shadow 0.2s,transform 0.2s;}}
...
<div class="header">
  <div class="header-left">
    ...
    <div class="sub">{card_name} · {cardholder} · {date_range}</div>
  </div>
  <div class="header-right">
    <div class="card-chip"><div class="chip-icon"></div><span class="card-num">{card_number}</span></div>
    <div class="card-sub">{card_name} · Auto-generated</div>
  </div>
</div>
<nav class="nav">
...
<div class="panel active" id="panel-overview">
  ...
  <div class="chart-card" style="margin-bottom:24px;">
    ...
    <div class="chart-sub">Hover cells for amount · deeper colour = higher spend</div>
...
<div class="panel" id="panel-yoy">
  ...
  <div class="chart-card" style="margin-bottom:24px;"><div class="chart-title">Annual Spend &amp; Reward Points</div><div class="chart-sub">Bars = spend · Line = points</div><canvas id="yoyBarChart" height="110"></canvas></div>
  <div class="chart-row cols-2">
    <div class="chart-card"><div class="chart-title">Category Mix by Year</div><div class="chart-sub">Stacked spend — top 10 categories</div><canvas id="yoyCatStack" height="220"></canvas></div>
...
<div class="panel" id="panel-mom">
  ...
  <div class="chart-card" style="margin-bottom:24px;"><div class="chart-title">Monthly Spend Timeline</div><div class="chart-sub">All months · use Year filter to zoom in</div><canvas id="momLineChart" height="110"></canvas></div>
  <div class="chart-row cols-2">
  ...
  <div class="chart-card" style="margin-bottom:24px;"><div class="chart-title">Category Mix by Month</div><div class="chart-sub">Stacked spend — top 10 categories · use Year filter to zoom in</div><canvas id="momCatStack" height="130"></canvas></div>
</div>
<div class="panel" id="panel-categories">
  ...
  <div class="chart-card"><div class="chart-title">All Subcategories</div><div class="chart-sub">Use Category filter to drill in</div><div class="detail-wrap" style="margin-top:16px;"><table class="data-table" id="subcat-table"></table></div></div>
</div>
<div class="panel" id="panel-points">
  ...
  <div class="chart-row cols-2">
    <div><div class="section-title">🏆 Best Point Returns <span style="font-size:10px;font-weight:400;color:var(--text3);text-transform:none;letter-spacing:0">pts/₹100 ↓ then spend ↓</span></div><div class="rate-grid" id="best-rates"></div></div>
    <div><div class="section-title">📉 Worst Point Returns <span style="font-size:10px;font-weight:400;color:var(--text3);text-transform:none;letter-spacing:0">incl. zero-earn · pts/₹100 ↑ then spend ↓</span></div><div class="rate-grid" id="worst-rates"></div></div>
  </div>
</div>
...
<div class="footer">
  ...
  <span>{card_name}</span>
  <span>Generated {today_str} · {date_range}</span>
</div>
<script>
const RAW={data_json};
...
const CAT_COLOR={{}};CATS.forEach((c,i)=>CAT_COLOR[c]=CAT_PALETTE[i%CAT_PALETTE.length]);
...
function applyFilters(){{
  const fYear=document.getElementById('fYear').value,fMonth=document.getElementById('fMonth').value,fCat=document.getElementById('fCat').value;
  const subcatSel=document.getElementById('fSubcat');const prev=subcatSel.value;
...
function resetFilters(){{['fYear','fMonth','fCat','fSubcat'].forEach(id=>document.getElementById(id).value='all');applyFilters();}}
let currentPanel='overview';
document.querySelectorAll('.nav-btn').forEach(btn=>{{btn.addEventListener('click',()=>{{document.querySelectorAll('.nav-btn').forEach(b=>b.classList.remove('active'));btn.classList.add('active');const p=btn.dataset.panel;document.querySelectorAll('.panel').forEach(el=>el.classList.remove('active'));document.getElementById('panel-'+p).classList.add('active');currentPanel=p;renderCurrent();}});}});
function renderCurrent(){{if(currentPanel==='overview')renderOverview();else if(currentPanel==='yoy')renderYOY();else if(currentPanel==='mom')renderMOM();else if(currentPanel==='categories')renderCategories();else if(currentPanel==='points')renderPoints();else if(currentPanel==='detail')renderDetail();}}
function destroyChart(id){{if(charts[id]){{charts[id].destroy();delete charts[id];}}}}
function aggRows(rows){{const r={{amt:0,pts:0,txns:rows.length,bycat:{{}},byyear:{{}},bymonth:{{}},bysubcat:{{}}}};rows.forEach(row=>{{r.amt+=row.amt;r.pts+=row.pts;r.bycat[row.cat]=(r.bycat[row.cat]||0)+row.amt;r.byyear[row.year]=(r.byyear[row.year]||0)+row.amt;const mk=`${{row.year}}-${{String(row.month).padStart(2,'0')}}`;r.bymonth[mk]=(r.bymonth[mk]||0)+row.amt;const sk=row.cat+'|||'+row.subcat;if(!r.bysubcat[sk])r.bysubcat[sk]={{amt:0,pts:0,txns:0,cat:row.cat,subcat:row.subcat}};r.bysubcat[sk].amt+=row.amt;r.bysubcat[sk].pts+=row.pts;r.bysubcat[sk].txns++;}});return r;}}
const gridOpts={{color:'rgba(20,20,19,0.06)'}};const tickOpts={{color:'#8a8880',font:{{size:11}}}};
function computeBestWorst(rows){{
  const vPts={{}};rows.forEach(r=>{{if(r.pts>0){{if(!vPts[r.desc])vPts[r.desc]={{amt:0,pts:0,count:0,cat:r.cat}};vPts[r.desc].amt+=r.amt;vPts[r.desc].pts+=r.pts;vPts[r.desc].count++;}}}});
  const best=[];Object.entries(vPts).forEach(([desc,v])=>{{if(v.count>=2&&v.amt>1000){{const rate=(v.pts/v.amt)*100;best.push({{desc:desc.slice(0,45),rate:+rate.toFixed(2),amt:+v.amt.toFixed(0),pts:v.pts,cat:v.cat}});}}}});best.sort((a,b)=>b.rate-a.rate||b.amt-a.amt);
  const vAll={{}};rows.forEach(r=>{{if(!vAll[r.desc])vAll[r.desc]={{amt:0,pts:0,count:0,cat:r.cat}};vAll[r.desc].amt+=r.amt;vAll[r.desc].pts+=r.pts;vAll[r.desc].count++;}});
  const worst=[];Object.entries(vAll).forEach(([desc,v])=>{{if(v.count>=2&&v.amt>1000){{const rate=(v.pts/v.amt)*100;worst.push({{desc:desc.slice(0,45),rate:+rate.toFixed(2),amt:+v.amt.toFixed(0),pts:v.pts,cat:v.cat}});}}}});worst.sort((a,b)=>a.rate-b.rate||b.amt-a.amt);
  return{{best:best.slice(0,12),worst:worst.slice(0,12)}};
}}
function rateCard(v,mode,rank){{const isZero=v.rate===0;const accentColor=mode==='best'?(v.rate>=3.3?'var(--green)':v.rate>=2.5?'var(--blue)':'var(--mid)'):(isZero?'var(--orange)':'#c87050');const ptsDisplay=isZero?'0':v.rate.toFixed(2);const barColor=mode==='best'?accentColor:(isZero?'rgba(217,119,87,0.8)':'rgba(200,112,80,0.6)');return`<div class="rate-card"><div class="rate-card-bar" style="background:${{barColor}};"></div><div class="rate-card-body"><div style="display:flex;align-items:flex-start;gap:6px;margin-bottom:4px;"><span class="rate-rank" style="background:${{barColor}}22;color:${{barColor}};font-size:9px;">#${{rank}}</span><div style="min-width:0;"><div class="rate-vendor" title="${{v.desc}}">${{v.desc}}</div><div class="rate-cat">${{v.cat}}${{isZero?' · <strong style="color:var(--orange)">Zero earn</strong>':''}}</div></div></div><div class="rate-bottom"><div class="rate-pts" style="color:${{accentColor}}">${{ptsDisplay}}<span class="rate-pts-unit"> pts/₹100</span></div><div class="rate-meta">${{fp(v.pts)}}<br>₹${{v.amt.toLocaleString('en-IN')}}</div></div></div></div>`;}}
function renderOverview(){{
  const a=aggRows(filteredRows);const invAmt=a.bycat['Investments & Assets']||0;const avgRate=a.amt>0?(a.pts/a.amt*100).toFixed(2):0;const months=Object.keys(a.bymonth).length||1;
  document.getElementById('kpi-grid').innerHTML=`<div class="kpi orange"><div class="kpi-accent"></div><div class="kpi-label">Total Spend</div><div class="kpi-value">${{fc(a.amt)}}</div><div class="kpi-sub">${{a.txns}} transactions</div></div><div class="kpi blue"><div class="kpi-accent"></div><div class="kpi-label">Lifestyle Spend</div><div class="kpi-value">${{fc(a.amt-invAmt)}}</div><div class="kpi-sub">excl. investments &amp; assets</div></div><div class="kpi green"><div class="kpi-accent"></div><div class="kpi-label">Points Earned</div><div class="kpi-value">${{fp(a.pts)}}</div><div class="kpi-sub">est. ₹${{(a.pts*0.5).toLocaleString('en-IN')}} @ 50p/pt</div></div><div class="kpi mid"><div class="kpi-accent"></div><div class="kpi-label">Avg Reward Rate</div><div class="kpi-value">${{avgRate}}</div><div class="kpi-sub">points per ₹100 spent</div></div><div class="kpi dark"><div class="kpi-accent"></div><div class="kpi-label">Avg Monthly</div><div class="kpi-value">${{fc(a.amt/months)}}</div><div class="kpi-sub">spend per active month</div></div>`;
  const allM={{}};RAW.rows.forEach(r=>{{const k=`${{r.year}}-${{String(r.month).padStart(2,'0')}}`;allM[k]=(allM[k]||0)+r.amt;}});const maxV=Math.max(...Object.values(allM));const years=[...new Set(RAW.rows.map(r=>r.year))].sort();
  let hm='<div class="heatmap-grid"><div></div>';for(let m=1;m<=12;m++)hm+=`<div class="hm-header">${{MONTHS[m]}}</div>`;
  years.forEach(y=>{{hm+=`<div class="hm-label">${{y}}</div>`;for(let m=1;m<=12;m++){{const k=`${{y}}-${{String(m).padStart(2,'0')}}`;const v=allM[k]||0;const t=v/maxV;const r2=Math.round(217*t+232*(1-t)),g2=Math.round(119*t+230*(1-t)),b2=Math.round(87*t+220*(1-t));const bg=v>0?`rgb(${{r2}},${{g2}},${{b2}})`:'#f3f1ea';const tc=t>0.55?'#fff':'#4a4840';hm+=`<div class="hm-cell" style="background:${{bg}};color:${{tc}};border-color:rgba(217,119,87,${{t*0.25}});" title="${{y}} ${{MONTHS[m]}}: ${{fc(v)}}">${{v>0?fc(v):''}}</div>`;}}}});hm+='</div>';document.getElementById('heatmap-container').innerHTML=hm;
  const yy=Object.keys(a.byyear).sort();destroyChart('yoyChart');charts['yoyChart']=new Chart(document.getElementById('yoyChart'),{{type:'bar',data:{{labels:yy,datasets:[{{label:'Annual Spend',data:yy.map(y=>a.byyear[y]),backgroundColor:'rgba(217,119,87,0.75)',borderRadius:6,borderSkipped:false}}]}},options:{{responsive:true,plugins:{{legend:{{display:false}},tooltip:{{callbacks:{{label:c=>fc(c.raw)}}}}}},scales:{{x:{{grid:gridOpts,ticks:tickOpts}},y:{{grid:gridOpts,ticks:{{...tickOpts,callback:v=>fc(v)}}}}}}  }}}});
  const tc=Object.entries(a.bycat).sort((a,b)=>b[1]-a[1]).slice(0,9);destroyChart('catPie');charts['catPie']=new Chart(document.getElementById('catPie'),{{type:'doughnut',data:{{labels:tc.map(([c])=>c),datasets:[{{data:tc.map(([,v])=>v),backgroundColor:tc.map(([c])=>CAT_COLOR[c]),borderWidth:2,borderColor:'#fff',hoverOffset:8}}]}},options:{{responsive:true,plugins:{{legend:{{position:'right',labels:{{font:{{size:11}},boxWidth:12,padding:10}}}},tooltip:{{callbacks:{{label:c=>c.label+': '+fc(c.raw)}}}}  }}}}}});
  const sorted=Object.entries(a.bycat).sort((a,b)=>b[1]-a[1]).slice(0,10);const mx=sorted[0]?.[1]||1;document.getElementById('cat-bars').innerHTML=sorted.map(([cat,amt])=>`<div class="bar-row"><div class="bar-label" title="${{cat}}">${{cat}}</div><div class="bar-track"><div class="bar-fill" style="width:${{(amt/mx*100).toFixed(1)}}%;background:${{CAT_COLOR[cat]}};"></div></div><div class="bar-val">${{fc(amt)}}</div></div>`).join('');
  const holders={{}};filteredRows.forEach(r=>{{holders[r.cardholder]=(holders[r.cardholder]||0)+r.amt;}});destroyChart('holderChart');charts['holderChart']=new Chart(document.getElementById('holderChart'),{{type:'doughnut',data:{{labels:Object.keys(holders).map(h=>h.split(' ')[0]),datasets:[{{data:Object.values(holders),backgroundColor:['rgba(217,119,87,0.8)','rgba(106,155,204,0.8)'],borderWidth:2,borderColor:'#fff'}}]}},options:{{responsive:true,plugins:{{legend:{{position:'bottom',labels:{{font:{{size:11}}}}}},tooltip:{{callbacks:{{label:c=>c.label+': '+fc(c.raw)}}}}  }}}}}});
}}
function renderYOY(){{
  const byYear={{}};filteredRows.forEach(r=>{{if(!byYear[r.year])byYear[r.year]={{amt:0,pts:0,txns:0,bycat:{{}}}};byYear[r.year].amt+=r.amt;byYear[r.year].pts+=r.pts;byYear[r.year].txns++;byYear[r.year].bycat[r.cat]=(byYear[r.year].bycat[r.cat]||0)+r.amt;}});const ay=Object.keys(byYear).sort();
  destroyChart('yoyBarChart');charts['yoyBarChart']=new Chart(document.getElementById('yoyBarChart'),{{type:'bar',data:{{labels:ay,datasets:[{{label:'Spend',data:ay.map(y=>byYear[y].amt),backgroundColor:'rgba(217,119,87,0.75)',borderRadius:6,borderSkipped:false,yAxisID:'y'}},{{label:'Points',data:ay.map(y=>byYear[y].pts),type:'line',borderColor:'#788c5d',backgroundColor:'rgba(120,140,93,0.08)',borderWidth:2.5,pointRadius:5,pointBackgroundColor:'#788c5d',yAxisID:'y2',tension:0.3}}]}},options:{{responsive:true,plugins:{{legend:{{labels:{{font:{{size:11}}}}}},tooltip:{{callbacks:{{label:c=>c.dataset.label+': '+(c.dataset.yAxisID==='y2'?fp(c.raw):fc(c.raw))}}}}}},scales:{{x:{{grid:gridOpts,ticks:tickOpts}},y:{{grid:gridOpts,ticks:{{...tickOpts,callback:v=>fc(v)}},position:'left'}},y2:{{grid:{{display:false}},ticks:{{...tickOpts,color:'#788c5d'}},position:'right'}}  }}}}}});
  const topC=Object.entries(RAW.by_cat).sort((a,b)=>b[1].amt-a[1].amt).slice(0,10).map(([c])=>c);
  destroyChart('yoyCatStack');charts['yoyCatStack']=new Chart(document.getElementById('yoyCatStack'),{{type:'bar',data:{{labels:ay,datasets:topC.map((cat,i)=>{{return{{label:cat,data:ay.map(y=>(byYear[y]?.bycat[cat])||0),backgroundColor:CAT_COLOR[cat]+'cc',borderRadius:i===0?6:0,stack:'s'}};}})}},options:{{responsive:true,plugins:{{legend:{{labels:{{font:{{size:10}}}}}},tooltip:{{callbacks:{{label:c=>c.dataset.label+': '+fc(c.raw)}}}}}},scales:{{x:{{stacked:true,grid:gridOpts,ticks:tickOpts}},y:{{stacked:true,grid:gridOpts,ticks:{{...tickOpts,callback:v=>fc(v)}}  }}}}}}}});
  document.getElementById('yoy-table').innerHTML='<tr><th>Year</th><th style="text-align:right">Spend</th><th style="text-align:right">Txns</th><th style="text-align:right">Points</th><th style="text-align:right">Rate</th></tr>'+ay.map(y=>{{const rate=byYear[y].amt>0?(byYear[y].pts/byYear[y].amt*100).toFixed(2):'0';return`<tr><td><span class="badge badge-orange">${{y}}</span></td><td class="num or" style="text-align:right">${{fc(byYear[y].amt)}}</td><td class="num dim" style="text-align:right">${{byYear[y].txns}}</td><td class="num gr" style="text-align:right">${{fp(byYear[y].pts)}}</td><td class="num bl" style="text-align:right">${{rate}} /₹100</td></tr>`;}}).join('');
}}
function renderMOM(){{
  const bm={{}};filteredRows.forEach(r=>{{const k=`${{r.year}}-${{String(r.month).padStart(2,'0')}}`;if(!bm[k])bm[k]={{amt:0,pts:0,txns:0,label:`${{MONTHS[r.month]}} ${{r.year}}`}};bm[k].amt+=r.amt;bm[k].pts+=r.pts;bm[k].txns++;}});const mk=Object.keys(bm).sort();
  destroyChart('momLineChart');charts['momLineChart']=new Chart(document.getElementById('momLineChart'),{{type:'line',data:{{labels:mk.map(k=>bm[k].label),datasets:[{{label:'Monthly Spend',data:mk.map(k=>bm[k].amt),borderColor:'#d97757',backgroundColor:'rgba(217,119,87,0.07)',borderWidth:2.5,pointRadius:3,pointBackgroundColor:'#d97757',fill:true,tension:0.4}}]}},options:{{responsive:true,plugins:{{legend:{{display:false}},tooltip:{{callbacks:{{label:c=>fc(c.raw)}}}}}},scales:{{x:{{grid:gridOpts,ticks:{{...tickOpts,maxTicksLimit:18}}}},y:{{grid:gridOpts,ticks:{{...tickOpts,callback:v=>fc(v)}}  }}}}}}}});
  const seasonal={{}};filteredRows.forEach(r=>{{seasonal[r.month]=(seasonal[r.month]||0)+r.amt;}});const sData=Array.from({{length:12}},(_,i)=>{{const m=i+1;const yrs=new Set(filteredRows.filter(r=>r.month===m).map(r=>r.year)).size;return yrs>0?(seasonal[m]||0)/yrs:0;}});
  destroyChart('seasonalChart');charts['seasonalChart']=new Chart(document.getElementById('seasonalChart'),{{type:'bar',data:{{labels:MONTHS.slice(1),datasets:[{{label:'Avg Monthly Spend',data:sData,backgroundColor:'rgba(106,155,204,0.7)',borderRadius:5,borderSkipped:false}}]}},options:{{responsive:true,plugins:{{legend:{{display:false}},tooltip:{{callbacks:{{label:c=>fc(c.raw)}}}}}},scales:{{x:{{grid:gridOpts,ticks:tickOpts}},y:{{grid:gridOpts,ticks:{{...tickOpts,callback:v=>fc(v)}}  }}}}}}}});
  const topM=Object.entries(bm).sort((a,b)=>b[1].amt-a[1].amt).slice(0,10);document.getElementById('top-months-table').innerHTML='<tr><th>#</th><th>Month</th><th style="text-align:right">Spend</th><th style="text-align:right">Txns</th></tr>'+topM.map(([k,v],i)=>`<tr><td class="dim num">${{i+1}}</td><td>${{v.label}}</td><td class="num or" style="text-align:right">${{fc(v.amt)}}</td><td class="num dim" style="text-align:right">${{v.txns}}</td></tr>`).join('');
  const topCats10=Object.entries(filteredRows.reduce((acc,r)=>{{acc[r.cat]=(acc[r.cat]||0)+r.amt;return acc;}},{{}})).sort((a,b)=>b[1]-a[1]).slice(0,10).map(([c])=>c);const catByMonth={{}};filteredRows.forEach(r=>{{const k=`${{r.year}}-${{String(r.month).padStart(2,'0')}}`;if(!catByMonth[k])catByMonth[k]={{}};catByMonth[k][r.cat]=(catByMonth[k][r.cat]||0)+r.amt;}});const allMK=Object.keys(catByMonth).sort();const allML=allMK.map(k=>{{const[y,m]=k.split('-');return MONTHS[+m]+' '+y;}});
  destroyChart('momCatStack');charts['momCatStack']=new Chart(document.getElementById('momCatStack'),{{type:'bar',data:{{labels:allML,datasets:topCats10.map((cat,i)=>{{return{{label:cat,data:allMK.map(k=>(catByMonth[k]?.[cat])||0),backgroundColor:CAT_COLOR[cat]+'cc',borderRadius:i===0?4:0,stack:'s'}};}})}},options:{{responsive:true,plugins:{{legend:{{labels:{{font:{{size:10}},boxWidth:12,padding:8}}}},tooltip:{{callbacks:{{label:c=>c.dataset.label+': '+fc(c.raw)}}}}}},scales:{{x:{{stacked:true,grid:gridOpts,ticks:{{...tickOpts,maxTicksLimit:24}}}},y:{{stacked:true,grid:gridOpts,ticks:{{...tickOpts,callback:v=>fc(v)}}  }}}}}}}});
}}
function renderCategories(){{
  const bc={{}},bs={{}};filteredRows.forEach(r=>{{if(!bc[r.cat])bc[r.cat]={{amt:0,pts:0,txns:0}};bc[r.cat].amt+=r.amt;bc[r.cat].pts+=r.pts;bc[r.cat].txns++;const sk=r.cat+'|||'+r.subcat;if(!bs[sk])bs[sk]={{amt:0,pts:0,txns:0,cat:r.cat,subcat:r.subcat}};bs[sk].amt+=r.amt;bs[sk].pts+=r.pts;bs[sk].txns++;}});
  const sorted=Object.entries(bc).sort((a,b)=>b[1].amt-a[1].amt);const mx=sorted[0]?.[1].amt||1;document.getElementById('all-cat-bars').innerHTML=sorted.map(([cat,v])=>`<div class="bar-row"><div class="bar-label" title="${{cat}}">${{cat}}</div><div class="bar-track"><div class="bar-fill" style="width:${{(v.amt/mx*100).toFixed(1)}}%;background:${{CAT_COLOR[cat]}};"></div></div><div class="bar-val">${{fc(v.amt)}}</div></div>`).join('');
  const catYrs=Object.keys(filteredRows.reduce((a,r)=>{{a[r.year]=1;return a;}},{{}})).sort();const topC=sorted.slice(0,6).map(([c])=>c);const cby={{}};filteredRows.forEach(r=>{{if(!cby[r.year])cby[r.year]={{}};cby[r.year][r.cat]=(cby[r.year][r.cat]||0)+r.amt;}});
  destroyChart('catTrendChart');charts['catTrendChart']=new Chart(document.getElementById('catTrendChart'),{{type:'line',data:{{labels:catYrs,datasets:topC.map((cat,i)=>{{const col=CAT_COLOR[cat];return{{label:cat,data:catYrs.map(y=>(cby[y]?.[cat])||0),borderColor:col,backgroundColor:'transparent',borderWidth:2.5,pointRadius:4,pointBackgroundColor:col,tension:0.3}};}})  }},options:{{responsive:true,plugins:{{legend:{{labels:{{font:{{size:10}}}}}},tooltip:{{callbacks:{{label:c=>c.dataset.label+': '+fc(c.raw)}}}}}},scales:{{x:{{grid:gridOpts,ticks:tickOpts}},y:{{grid:gridOpts,ticks:{{...tickOpts,callback:v=>fc(v)}}  }}}}}}}});
  const scSorted=Object.values(bs).sort((a,b)=>b.amt-a.amt);const tAmt=scSorted.reduce((s,v)=>s+v.amt,0)||1;document.getElementById('subcat-table').innerHTML='<tr><th>Category</th><th>Subcategory</th><th style="text-align:right">Spend</th><th style="text-align:right">Share</th><th style="text-align:right">Txns</th><th style="text-align:right">Points</th></tr>'+scSorted.map(v=>`<tr><td><span class="badge" style="background:${{CAT_COLOR[v.cat]}}18;color:${{CAT_COLOR[v.cat]}}">${{v.cat}}</span></td><td style="color:var(--text2)">${{v.subcat}}</td><td class="num or" style="text-align:right">${{fc(v.amt)}}</td><td class="num dim" style="text-align:right">${{(v.amt/tAmt*100).toFixed(1)}}%</td><td class="num dim" style="text-align:right">${{v.txns}}</td><td class="num gr" style="text-align:right">${{v.pts}}</td></tr>`).join('');
}}
function renderPoints(){{
  const tot=filteredRows.reduce((s,r)=>s+r.amt,0);const pts=filteredRows.reduce((s,r)=>s+r.pts,0);const rate=tot>0?(pts/tot*100).toFixed(2):0;const zero=filteredRows.filter(r=>r.pts===0&&r.amt>0);const zeroAmt=zero.reduce((s,r)=>s+r.amt,0);
  document.getElementById('pts-kpi-grid').innerHTML=`<div class="kpi green"><div class="kpi-accent"></div><div class="kpi-label">Total Points</div><div class="kpi-value">${{fp(pts)}}</div><div class="kpi-sub">est. ₹${{(pts*0.5).toLocaleString('en-IN')}} value</div></div><div class="kpi orange"><div class="kpi-accent"></div><div class="kpi-label">Avg Reward Rate</div><div class="kpi-value">${{rate}}</div><div class="kpi-sub">points per ₹100 spent</div></div><div class="kpi blue"><div class="kpi-accent"></div><div class="kpi-label">Zero-Earn Spend</div><div class="kpi-value">${{fc(zeroAmt)}}</div><div class="kpi-sub">${{zero.length}} txns · ${{(zeroAmt/tot*100).toFixed(1)}}% of total</div></div><div class="kpi mid"><div class="kpi-accent"></div><div class="kpi-label">Missed Points</div><div class="kpi-value">${{fp(Math.round(zeroAmt*3.33/100))}}</div><div class="kpi-sub">potential @ 3.33/₹100</div></div>`;
  const pby={{}};filteredRows.forEach(r=>{{pby[r.year]=(pby[r.year]||0)+r.pts;}});const py=Object.keys(pby).sort();destroyChart('ptsYoyChart');charts['ptsYoyChart']=new Chart(document.getElementById('ptsYoyChart'),{{type:'bar',data:{{labels:py,datasets:[{{label:'Points Earned',data:py.map(y=>pby[y]),backgroundColor:'rgba(120,140,93,0.75)',borderRadius:6,borderSkipped:false}}]}},options:{{responsive:true,plugins:{{legend:{{display:false}},tooltip:{{callbacks:{{label:c=>fp(c.raw)}}}}}},scales:{{x:{{grid:gridOpts,ticks:tickOpts}},y:{{grid:gridOpts,ticks:tickOpts}}  }}}}}}}});
  const cr={{}};filteredRows.forEach(r=>{{if(!cr[r.cat])cr[r.cat]={{amt:0,pts:0}};cr[r.cat].amt+=r.amt;cr[r.cat].pts+=r.pts;}});const crs=Object.entries(cr).filter(([,v])=>v.amt>0).map(([c,v])=>([c,(v.pts/v.amt*100)])).sort((a,b)=>b[1]-a[1]);destroyChart('ptsRateChart');charts['ptsRateChart']=new Chart(document.getElementById('ptsRateChart'),{{type:'bar',data:{{labels:crs.map(([c])=>c),datasets:[{{label:'Pts/₹100',data:crs.map(([,r])=>+r.toFixed(2)),backgroundColor:crs.map(([c])=>CAT_COLOR[c]+'cc'),borderRadius:4,borderSkipped:false}}]}},options:{{indexAxis:'y',responsive:true,plugins:{{legend:{{display:false}},tooltip:{{callbacks:{{label:c=>c.raw+' pts/₹100'}}}}}},scales:{{x:{{grid:gridOpts,ticks:tickOpts}},y:{{grid:{{display:false}},ticks:{{...tickOpts,font:{{size:10}}}}  }}}}}}}});
  const{{best,worst}}=computeBestWorst(filteredRows);document.getElementById('best-rates').innerHTML=best.map((v,i)=>rateCard(v,'best',i+1)).join('');document.getElementById('worst-rates').innerHTML=worst.map((v,i)=>rateCard(v,'worst',i+1)).join('');
}}
...
def regenerate_report(ledger_path: str, config: dict, output_path: str) -> str:
    """
    Full report regeneration pipeline.
